...
async def get_alias(
    ...
    redis_state_model = validate_redis_state(
        redis_state=await get_registration(url_token=token, redis=redis),
        model_class=AfterCreationState,
    )

    if redis_state_model is None:
        await delete_registration(url_token=token, redis=redis)
        raise HTTPException(
    ...
    if not await update_registration(url_token=token, new_url_token=new_token, mapping={"valid": "true"}, redis=redis):
        raise HTTPException(
...
async def post_alias(
    ...
    new_token = token_urlsafe(64)
    if not await update_registration(
        url_token=registration_token, new_url_token=new_token, mapping={"alias": alias}, redis=redis
    ):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=gettext("Internal server error."),
